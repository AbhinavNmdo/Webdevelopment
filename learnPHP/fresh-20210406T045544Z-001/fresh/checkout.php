...
if (is_array($d)) {
  ...
  foreach ($rows as $row) {
    ...
    $fn = $new_array['firstName'];
    $ln = $new_array['lastName'];
    $adr = $new_array['address'] . ' ' . $new_array['neighborhood'] . ' ' . $new_array['sublocality'];
    $city = $new_array['city'];
    $state = $new_array['state'];
    $zip = $new_array['postcode'];
    $country = $new_array['country'];
    $telephone = $new_array['phone'];

    array_push($_SESSION['user_details'], $fn);
    array_push($_SESSION['user_details'], $ln);
    array_push($_SESSION['user_details'], $adr);
    array_push($_SESSION['user_details'], $city);
    array_push($_SESSION['user_details'], $state);
    array_push($_SESSION['user_details'], $zip);
    array_push($_SESSION['user_details'], $country);
    array_push($_SESSION['user_details'], $telephone);
...
<html lang="en">
   <head>
      ...
      <title>thedailyfreshstore | Checkout</title>
      <!-- Mobile Specific -->
                     ...
                     <div class="col-xs-6 hidden-xs">
                        ...
                        <div class="toplinks">
                           <div class="links">
                              ...
                              <div class="check"><a title="Checkout" href="checkout.php?logout=true"><span class="hidden-xs">Logout</span></a> </div>
                           </div>
                        </div>
...
?>
    ...
    <div class="main container">
      <div class="row">
        <section class="col-sm-9 wow bounceInUp animated load_shipping_page">
        <!--  Call here Shipping method and payments method -->
        </section>
          
          <aside class="col-right sidebar col-sm-3 wow bounceInUp animated" style=" color: #111;
          ...
          <div class="block block-progress">
            <div class="block-title ">Your Billings</div>
  ...
  <!--End main-container --> 
      ...
      <script>
        ...
        {
          $.ajax({
            url:"Shipping_method.php",
            method:"POST",
            success:function(data)
            {
              $('.load_shipping_page').html(data);
            }
            });
        }
...
//  sending data to Pay.php for payments gateway

$(document).on('click', '.continue_to_pay', function(){

  var user_details = <?php echo $_SESSION['user_details']; ?>;
   var amount = $('#order_final_price').val();
   
   $.ajax({
      url:"shipping_payment_method.php",
      method:"POST",
      data:{user_details:user_details, amount:amount},
      success:function(data)
      {
        $('.load_shipping_page').html(data);
        document.body.scrollTop = 0; // For Safari
